...
/// \brief 诱导排序.
/// \param s 字符串数组, `0` 为最小值且以 `0` 结尾.
/// \param K 字符集值域为 \f$[0, K)\f$
/// \remarks 时间复杂度为 \f$O(n)\f$.
...
/// \brief 获取后缀数组.
/// \param s 为字符串.
std::vector<int> test::suffix_array(const std::string &s) { return detail::suffix_array(s); }

/// \brief 获取后缀数组.
/// \param s 为字符串数组, 其中字符都大于零.
std::vector<int> test::suffix_array(const std::vector<int> &s) { return detail::suffix_array(s); }